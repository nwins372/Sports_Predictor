...
function SportsSelection({ initialSelected = "", onSelect = () => {} }) {
  ...
  useEffect(() => {
    const handleSportChanged = (e) => {
      if (e?.detail) {
        setSelected(e.detail); // ✅ use setSelected
      }
    };
    window.addEventListener("sportChanged", handleSportChanged);
    return () => window.removeEventListener("sportChanged", handleSportChanged);
  }, []);
  ...
  const sports = [
    ...
    { name: "Baseball", img: "https://upload.wikimedia.org/wikipedia/commons/1/1e/Baseball_%28icon%29.png" },
  ];

  const handleSelect = (name) => {
    setSelected(name);
    const sportKey = name.toLowerCase();
    localStorage.setItem("selectedSport", sportKey);

    // Dispatch the new sport in the event
    window.dispatchEvent(new CustomEvent("sportChanged", { detail: sportKey }));
  ...
  return (
    <div className="sports-selection" style={{ padding: 20 }}>
          ...
          return (
            <div
              ...
              style={{
                cursor: "pointer",
                background: "var(--bg)",
                border: `4px solid ${isSelected ? "var(--accent)" : "var(--muted)"}`,
                borderRadius: 18,
      ...
      {selected && (
        ...
        >
          <h3 style={{ margin: 0, fontSize: 16 }}>You selected: {selected} 🏆</h3>
        </div>
      )}
    </div>
  );
